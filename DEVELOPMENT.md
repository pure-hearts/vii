...
本指南旨在帮助开发者快速熟悉 `vii` 仓库的项目结构、本地开发调试流程、基于 `vite-plus` 的构建设计以及工具的具体使用方法。

---

## 1. 项目结构 (Project Structure)

项目是一个基于 **pnpm workspace** 的 Monorepo 仓库，由 `vite-plus` (`vp`) 驱动统一的构建和校验。
...
## 2. 开发指南 (Development Guide & Debugging)
...
相较于原先 the `unbuild`，新配置文件基于 Vite 标准进行拓展，支持模块别名（alias）与打包配置（`pack` 属性）：
- **`packages/utils`**：打包输出 ESM (`.mjs`) 与 CJS (`.cjs`)，并自动生成双类型 TS 声明文件 (`.d.mts`, `.d.cts`)。
